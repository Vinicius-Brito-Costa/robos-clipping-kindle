package com.lionfish.robo_clipping_kindle.filter;
...
/**
 * Filters once per request validating the JWT token
 */
@Component
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if(authHeader != null && authHeader.contains(BEARER) && SecurityContextHolder.getContext().getAuthentication() == null){
            ...
            if(JWTUtil.validateToken(jwt)){
                UserDetails userDetails = userDetailService.loadUserByUsername(JWTUtil.getUsername(jwt));
